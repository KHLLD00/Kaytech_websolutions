...
export const metadata: Metadata = {
  ...
  description:
    "Explore selected website design and development projects built by Kaytech Web Solutions.",
  alternates: {
    canonical: "/projects",
  },
};
          ...
          {rows.map((p: any) => (
            <div key={p.id}>
              <div className="relative aspect-[16/10] overflow-hidden rounded-card border border-[var(--color-border)] bg-[var(--color-surface)]">
                {p.fallback_image ? (
                  <img
                    src={p.fallback_image}
                    alt={p.name}
                    className="absolute inset-0 h-full w-full object-cover"
                  />
                ) : (
                  <div className="h-full w-full bg-gradient-to-br from-[#2563EB] to-[#7C3AED] opacity-90" />
                )}
                {p.live_url && (
                  <iframe
                    src={p.live_url}
                    title={p.name}
                    className="absolute inset-0 h-full w-full border-0 bg-[var(--color-surface)]"
                    loading="lazy"
                  />
                )}
              </div>
              ...
              {p.live_url && (
                <Button href={p.live_url} variant="secondary" className="mt-4">
                  Visit Website
                </Button>
              )}
            </div>
